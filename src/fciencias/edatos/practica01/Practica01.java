...
* @author Erick Iram García Velasco - 318044309
...
public class Practica01{
	...
	* @return un arreglo ordenado de longitud m+n con la mezcla definida.
	*/
	 public static int[] mergeSortedArrayV2(int[] array1, int n, int[] array2, int m){
		if(n > array1.length || m > array2.length)
	 		throw new RuntimeException("Límites no válidos");
		
		//Un arreglo nuevo que será el resultado
		//Su longitud es la suma de los límtes definidos
		int[] arrFinal = new int[n + m];

		//Un for para recorrer y comparar los elementos de los arreglos
		//n para el array 1
		//m para el aray 2
		//k indica dónde se guarda el elemento más chico de cada operación en el arreglo final
	 	for(int i = 0, j = 0, k = 0; i <= n && j <= m; i++, j++, k++){

			//comparamos los primeros elementos de cada arreglo y guaradamos el más pequeño
			//usamos un if porque solo hay dos casos
	 		if(array1[i]<array2[j]){
	 			arrFinal[k] = array1[i];
				 if(j!=0){
			// le restamos 1 a j para que compare el mismo elemento y no el siguiente
					 j--;	
				 }
	 		}else{
	 			...
	 			arrFinal[k] = array2[j];
				 if(i!=0){
			// le restamos 1 a i para que compare el mismo elemento y no el siguiente
					 i--;
				 }
	...
	public static boolean isValidBoardPlus(int [][] board){
		...
		for (int i = 0; i < length ; i++) {
			...
			for (int j = 0; j < length ; j++ ) {
				...
				// esto quiere decir, que ya aparezcan en mi lista fila
				if(fila[board[i][j]] == false){
					// En caso de que el elemento aparezca por primera vez, 
				...
				// esto quiere decir, que ya aparezcan en mi lista columna
	...
	*/
	...
	/**Calula el tiempo de ejecución de los algoritmos del ejercicio 1
	 *@param directorio ruta de acceso a al archivo .txt del arreglo que se utilizará en la prueba
	 *@param array1 string que se convertirá a arreglo
	 *@param array2 string que se convertirá a arreglo
	 *@param n límite del arreglo uno 
	 *@param m límite del arreglo dos
	 */
	public static void calcularTiempoEjercicio1(String directorio,String array_1,int n, String array_2, int m ){
		//se inicializan los arreglos para la prueba
		int[] array1 = ArrayReader.readArray(directorio + array_1);
		int[] array2 = ArrayReader.readArray(directorio + array_2);

		//Prueba con el algoritmo no mejorado
		long inicio1 = System.currentTimeMillis();
		mergeSortedArray(array1, n, array2, m);
		long fin1 = System.currentTimeMillis();

		//Prueba con el algoritmo mejorado
		long inicio2 = System.currentTimeMillis();
		mergeSortedArrayV2(array1, n, array2, m);
		long fin2 = System.currentTimeMillis();

		//Presenta en terminal los resultados de los algortimos
		System.out.println("\n---------------------------------------------------\n");
		System.out.println("El algoritmo tardó " + (fin1 - inicio1) + " milisegundos");
		System.out.println("El algoritmo mejorado tardó " + (fin2 - inicio2) + " milisegundos");
	...
	*/
	...
	/**
	* Calcula el tiempo de ejecución de los algoritmos correspondientes al ejercicio 3.
	...
	*/
	...
	public static void main(String[] args) {
		...
		calcularTiempoEjercicio1(directorio1,"ArrayA1.txt", 3, "ArrayA2.txt", 5);
		...
		calcularTiempoEjercicio1(directorio1, "ArrayB1.txt", 5, "ArrayB2.txt", 5);
		...
		calcularTiempoEjercicio1(directorio1, "ArrayC1.txt", 4, "ArrayC2.txt", 6);
